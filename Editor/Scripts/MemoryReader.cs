...
namespace HeapExplorer
{
    ...
    abstract public class AbstractMemoryReader
    {
        ...
        Option<System.ArraySegment<byte>> ReadObjectBytes(ulong address, PackedManagedType typeDescription) {
            if (!ReadObjectSize(address, typeDescription).valueOut(out var size)) return None._;
            if (size <= 0) {
        ...
        public Option<int> ReadObjectSize(ulong address, PackedManagedType typeDescription)
        {
            // System.Array
